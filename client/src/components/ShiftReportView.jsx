...
export default function ShiftReportView({ data, header, selectedMachines, SHIFT_ROWS = [], docStatus }) {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 text-slate-900 font-sans border border-slate-300 shadow-sm mx-auto relative overflow-hidden" style={{ width: '100%', maxWidth: '1100px' }}>
      
      {/* Optional: PENDING Watermark if not fully verified */}
      {(!safeStatus?.verifiedBy) && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-45 text-slate-100 text-[6rem] sm:text-9xl font-black select-none pointer-events-none opacity-40 z-0">
          PENDING
        </div>
      )}
      ...
      <div className="relative z-10 overflow-x-auto scrollbar-thin">
        <div className="min-w-[850px] space-y-4">
          ...
          <table className="w-full border-collapse border-2 border-slate-900 text-[11px]">
            <tbody>
              <tr>
                ...
                <td className="border-2 border-slate-900 p-1 font-bold w-[120px]">Issue/ Rev. No.</td>
                <td className="border-2 border-slate-900 p-1 text-center font-bold w-[80px]">01/00</td>
              </tr>
              <tr>
          ...
          {/* Signature Section */}
          <div className="mt-12 flex flex-col sm:flex-row justify-between gap-8 px-4 sm:px-10">
